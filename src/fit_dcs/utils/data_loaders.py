...
from fit_dcs.core.lib_loader import _HAS_C_LIB
...
class DataLoaderTimeTagger:
    """
    Data loader for the .ttbin files created by the Time Tagger. Requires the Time Tagger Python API.
    ...
    def __init__(
        ...
        """
        if not _HAS_TIMETAGGER:
            raise RuntimeError("Time Tagger Python API is not installed. Please install it to use this class.")
        self.data_file_paths = data_file_paths
    ...
    def _process_channel(self, i_channel, tt):
        ...
        if _HAS_C_LIB:
            ...
            g2_norm_out, tau_out = async_corr(
                np.array(tt[i_channel]),
                **self.correlator_args
            )
