...
def g1(msd: np.ndarray, mua: float, musp: float, r: float, lambda0: float) -> np.ndarray:
    """
    Calculates the unnormalized first-order autocorrelation function g1 for a homogeneous infinite medium.
    ...
    :param r: Source-detector separation. [cm]
    ...
    """
    k0 = 2 * np.pi / (lambda0 * 1e-7) # Convert lambda0 to cm
    k = np.sqrt(3 * musp * mua + musp**2 * k0**2 * msd)

    return np.exp(-k * r) / r


def g1_norm(msd: np.ndarray | float, mua: float, musp: float, r: float, lambda0: float) -> np.ndarray:
    """
    Calculates the normalized first-order autocorrelation function g1 for a homogeneous infinite medium.
    ...
    """
    return g1(msd, mua, musp, r, lambda0) / g1(0, mua, musp, r, lambda0)


def d_factors(msd0: np.ndarray, mua0: float, musp0: float, r: float, lambda0: float) \
        -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Calculates the d factors for the DCS Modified Beer-Lambert law for the homogeneous infinite medium. See [1] for
    an explanation.
    ...
    dr = musp0**2 * k0**2 * r / k
    da = 3 * musp0 * r * (1 / k - 1 / mu_eff)
    ds = r * ((3 * mua0 + 2 * k0**2 * musp0 * msd0) / k - 3 * mua0 / mu_eff)
...
if __name__ == '__main__':
    ...
    (dr, da, ds) = d_factors(msd, mua, musp, r, lambda0)
    ...
    plt.title("Sensitivity factors for homogeneous infinite medium")
    plt.show()
