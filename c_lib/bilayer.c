...
double g1_spatial_freq(
    const double msd_up,  // Mean-square displacement in the upper layer [cm^2/s]
    const double msd_dn,  // Mean-square displacement in the lower layer [cm^2/s]
...
double integrand(int n, const double *x, const void *params) {
    ...
    const double msd_dn = p[1];
    const double mua_up = p[2];
    const double mua_dn = p[3];
    const double musp_up = p[4];
    ...
    const double g1_sf = g1_spatial_freq(msd_up, msd_dn, mua_up, mua_dn, musp_up, musp_dn, n_refractive, d, lambda0, q);
